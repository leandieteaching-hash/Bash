...
export async function GET(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({data: {id: session.userId, email: session.email, activeOrganisationId: session.organisationId, organisations: await listOrganisations(session.userId)}, requestId: context.requestId}, {headers: {'x-request-id': context.requestId}});
  } catch (error) {
